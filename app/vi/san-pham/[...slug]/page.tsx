...
// Tạo breadcrumb paths từ danh sách menu (mỗi menu là một đường dẫn riêng)
...
> {
  ...
  return webMenus
    ...
    .filter(Boolean)
    .map((menu) => [menu]) // Mỗi menu là một breadcrumb riêng (giữ nguyên UI cũ)
}

export default async function ProductPage({ params }: PageProps) {
  const { slug } = await params
  const product = await getProductBySlug(slug, 'vi')
